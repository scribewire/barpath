...
def simulate_clean_jerk_sequence(clean_csv: Path, jerk_csv: Path) -> dict:
    ...
    for idx, row in df_clean.iterrows():
        ...
        lm = {}
        for name, mp in [("left_shoulder", 11), ("right_shoulder", 12), ("left_hip", 23), ("right_hip", 24), ("left_knee", 25), ("right_knee", 26)]:
...
if __name__ == "__main__":
    clean_csv = Path("outputs/male/clean/botev_10_clean/final_analysis.csv")
    jerk_csv = Path("outputs/male/jerk/botev_15_jerk/final_analysis.csv")
    ...
    print(f"  Final: state={result['final_state']} stack={result['final_stack']} predicted={result['predicted']}")
    ...
    print(f"  Final: stack={rec._display_stack} predicted={rec._predicted_class} state={rec.state.name}")

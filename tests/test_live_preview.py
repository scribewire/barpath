...
from barpath.pipeline.realtime_processing.live_feature_extractor import (
...
def get_csv_files(category: str, max_per_class: int = 20) -> list[Path]:
    ...
    base = Path(f"outputs/dataset/male/{category}")
    if not base.exists():
        return []
    csv_files = []
...
def run_test_harness(
    ...
    if snatch_files:
        correct, total, preds = _run_category_test(

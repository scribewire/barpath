...
def test_progress_flow():
    ...
    if len(step1_at_100) == 1:
        print(f"✓ Step 1 has exactly 1 yield at 100% (no duplicates)")
    else:
        print(f"✗ Step 1 has {len(step1_at_100)} yields at 100% (should be 1)")
    
    if len(step4_at_100) == 1:
        print(f"✓ Step 4 has exactly 1 yield at 100% (no duplicates)")
    else:
    ...
    if step1_progress == sorted(step1_progress):
        print(f"✓ Step 1 progress is monotonically increasing")
    else:
        print(f"✗ Step 1 progress is not monotonic")
    
    if step4_progress == sorted(step4_progress):
        print(f"✓ Step 4 progress is monotonically increasing")
    else:
        print(f"✗ Step 4 progress is not monotonic")

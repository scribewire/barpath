...
class LiftDetectionSystem:
    ...
    def __init__(
        ...
        self.buffer = CircularFrameBuffer(
            max_duration_ms=self.BUFFER_DURATION_MS,
            fps=fps
        )
    ...
    def _handle_detecting(self) -> Optional[Dict]:
        ...
        predicted_class = self.current_result.get('class', 'none')
    ...
    def _handle_complete(self) -> Optional[Dict]:
        ...
        if self.current_result['confidence'] < self.CONFIDENCE_THRESHOLD:
    ...
    def _finalize_detection(self) -> Dict:
        ...
        if predicted_class == 'clean':
            self.pending_clean_result = self.current_result.copy()
    ...
    def status_text(self) -> str:
        ...
        if self.state == DetectionState.IDLE:
            ...
            cls = self.current_result.get('class', 'unknown')
            conf = self.current_result.get('confidence', 0)
            ...
            cls = self.current_result.get('class', 'unknown')
            return f"{cls.upper()} ({self.current_result.get('confidence', 0):.0%})"
        return ""

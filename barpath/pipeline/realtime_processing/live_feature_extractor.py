...
def _safe_float(value: float, default: float = 0.0) -> float:
    ...
    result = float(value) if isinstance(value, (int, float, np.number)) else default
...
class LiveFeatureExtractor:
    ...
    def _compute_kinematics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(timestamps) > 1:
            ...
            df["vel_y_smooth"] = _safe_savgol(
                _to_float_array(vel), max_win=min(9, len(vel)), polyorder=2
            )
        else:
            df["vel_y_smooth"] = 0.0

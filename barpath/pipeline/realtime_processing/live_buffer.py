...
class FrameData:
    ...
    timestamp_ms: float
    barbell_center: Optional[Tuple[float, float]] = None  # (x, y) in pixels
    barbell_box: Optional[Tuple[int, int, int, int]] = None  # (x1, y1, x2, y2)
    landmarks: Dict[int, Tuple[float, float, float, float]] = field(default_factory=dict)  # MediaPipe idx -> (x, y, z, visibility)
    knee_y_avg: float = 0.0  # Average knee y position in pixels
    joint_angles: Dict[str, float] = field(default_factory=dict)  # 'left_knee', 'right_knee', etc. in degrees
...
class CircularFrameBuffer:
    ...
    def get_window(self, duration_ms: float, 
                 end_time_ms: Optional[float] = None) -> List[FrameData]:
        """
    ...
    def get_barbell_positions(self, frames: Optional[List[FrameData]] = None) -> np.ndarray:
        """
    ...
    def get_barbell_y(self, frames: Optional[List[FrameData]] = None) -> np.ndarray:
    ...
    def get_barbell_velocities(self, frames: Optional[List[FrameData]] = None) -> np.ndarray:
    ...
    def get_knee_angles(self, frames: Optional[List[FrameData]] = None) -> np.ndarray:
    ...
    def get_elbow_angles(self, frames: Optional[List[FrameData]] = None) -> np.ndarray:
    ...
    def get_timestamps(self, frames: Optional[List[FrameData]] = None) -> np.ndarray:

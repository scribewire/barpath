...
try:
    ...
    from step4_helpers.smart_analysis import (
    ...
    from barpath.pipeline.step4_helpers.smart_analysis import (
        load_smart_analysis_model,
        run_smart_analysis,
    )
...
def get_phase_names(lift_type: str) -> dict[int, str]:
    ...
    return PHASE_NAMES.get(lift_type.lower(), {0: "Phase 0", 1: "Phase 1", 2: "Phase 2"})
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            if faults:
                if score is not None:
                    ...
                    f.write(f"**Technique Score:** {score:.0f}/100 {stars} ({label})\n\n")
                ...
                if clear:
                    ...
                    for fid in clear:
                        name = FAULT_DEFS.get(fid, {}).get("name", fid.replace("_", " ").title())
                        f.write(f"- [x] {name}\n")
                    f.write("\n")
            else:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            if clean_result:
                ...
                if faults:
                    ...
                    for fault in faults:
                        conf = fault.get("confidence", 0)
                        f.write(f"- **{fault.get('name', 'Unknown')}** ({conf}% confidence)\n")
...
def _get_all_fault_ids_for_lift(lift_type: str) -> list[str]:
    ...
    return [fid for fid, fdef in FAULT_DEFS.items() if lift_type in fdef.get("lift_types", [])]
...
def _analyze_segment(
    ...
    """Analyze a single lift segment.

    Tries the ML Smart Analysis model first (Random Forest fault detection);
    falls back to the compiled rule-based analyzer when no trained model is
    present, matching the documented ML → compiled → skip chain.
    """
    ...
    # Optional ML tier: Random Forest fault detection, if a trained model ships.
    ml_faults: list[dict[str, Any]] = []
    smart_model_dir = models_base / lifter / lift_type
    model, features_config, faults_config = load_smart_analysis_model(smart_model_dir)
    if model is not None:
        ml_result = run_smart_analysis(features, model, features_config, faults_config)
        if ml_result.get("available"):
            for fid, prob in ml_result["fault_probabilities"].items():
                fdef = FAULT_DEFS.get(fid, {})
                if fdef and prob >= 0.5:
                    ml_faults.append(
                        {
                            "id": fid,
                            "name": fdef.get("name", fid),
                            "confidence": round(prob * 100, 1),
                            "phase": _get_fault_phase(fid, lift_type),
                            "description": fdef.get("description", ""),
                            "coaching_cue": fdef.get("coaching_cue", ""),
    ...
    return {
        ...
        "features": features,
        "smart_analysis_used": bool(ml_faults),
    }
...
def critique_lift(
    ...
    if not has_valid_phases:
        logger.warning("CSV missing phases in bar_phase column — analysis may be unreliable")
    ...
    if lift_type == "clean_jerk" and "lift_segment" in df.columns:
        ...
        if len(df_clean) > 0:
            clean_result = _analyze_segment(cast(pd.DataFrame, df_clean), "clean", lifter, gender)
            all_faults.extend(clean_result["faults"])

        if len(df_jerk) > 0:
            jerk_result = _analyze_segment(cast(pd.DataFrame, df_jerk), "jerk", lifter, gender)
...
def main() -> None:
    ...
    parser.add_argument("--lifter", default="generic", help="Lifter name for baseline selection.")
    parser.add_argument(
        "--gender",
        default="male",
        choices=["male", "female"],
    ...
    parser.add_argument("--output_dir", default=".", help="Output directory for analysis.md")
    ...
    try:
        ...
        logger.info(f"Analysis complete. Results saved to {args.output_dir}/analysis.md")

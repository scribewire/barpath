...
def truncate_at_knee_pass(
    ...
    if not df["barbell_y_smooth"].notna().any() or not df["knee_y_avg"].notna().any():
...
def truncate_at_peak_height(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        Truncated DataFrame
    """
    if not df["barbell_y_smooth"].notna().any():
...
def detect_three_phases(df: pd.DataFrame, fps: float) -> pd.Series:
    ...
    hip_drop_threshold = (
        ...
        if float(hip_smooth.std()) > 0 else 0.5
    )
...
def assign_phases_from_classics(
    df: pd.DataFrame,
    phases: dict
) -> pd.DataFrame:
    """

...
def install_with_input():
    ...
    if onnx_packages:
        ...
        print("  [1] ONNX Runtime Acceleration")
        print(f"      Package(s): {pkg_str}")
        if os_type == "windows" and "directml" in pkg_str:
            print("      GPU acceleration via DirectML (supports NVIDIA/AMD/Intel GPU)")
        elif os_type == "macos":
            print("      GPU acceleration via Apple Metal (faster inference)")
        elif os_type == "linux" and "gpu" in pkg_str:
            print("      NVIDIA GPU acceleration via CUDA")
        elif os_type == "linux" and "rocm" in pkg_str:
            print("      AMD GPU acceleration via ROCm")
        else:
            print("      CPU optimization via ONNX Runtime")
        print()

    if openvino_packages:
        has_options = True
        pkg_str = ", ".join(openvino_packages)
        print("  [2] OpenVINO Acceleration")
        print("      Package(s): {pkg_str}")
        print("      Intel CPU optimization (faster inference on Intel processors)")
        print()
    ...
    if packages_to_install:
        ...
        if "openvino" in " ".join(packages_to_install):
            if (
                "onnxruntime" in " ".join(packages_to_install)
                or "onnxruntime-directml" in " ".join(packages_to_install)
                or "onnxruntime-metal" in " ".join(packages_to_install)
                or "onnxruntime-gpu" in " ".join(packages_to_install)
                or "onnxruntime-rocm" in " ".join(packages_to_install)
            ):
                print("  pip install .[hardware]\n")
            else:
